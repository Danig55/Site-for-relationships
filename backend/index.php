<?php
// Run with:  php -S localhost:8000 -t backend
...
function feed(): void {
    ...
    $params = $excluded;
    if ($me['InterestedIn'] !== 'everyone') { $sql .= ' AND Gender = ?'; $params[] = $me['InterestedIn']; }
    $candidates = all($sql, $params);

    $myAge = calc_age($me['Birthdate']);
    $out = [];
    foreach ($candidates as $c) {
        $age = calc_age($c['Birthdate']);
        if ($age < $me['MinAge'] || $age > $me['MaxAge']) continue;
        if ($c['InterestedIn'] !== 'everyone' && $c['InterestedIn'] !== $me['Gender']) continue;
        if ($myAge < $c['MinAge'] || $myAge > $c['MaxAge']) continue;
        $out[] = user_to_feed($c);
    }
    json_out($out);
}
